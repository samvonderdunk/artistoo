...
def readfile(fname, selector, verbose=True, start=None, stop=None,  reverse=False, **kwargs):
...
def openruns(fname='Mitochondrialog.txt', folder='current', runs=None, selector=None, verbose=False, sortbykeywordix=[], sortbylineix=[], longform=False, **kwargs):
    ...
    for ix, subdir in  enumerate(sorted([os.path.join(folder, dirname) for dirname in runs])):
        path = os.path.join(subdir, fname)
        if verbose:
            print(path, ix) 
        df = readfile(path, selector, verbose=verbose, **kwargs)
        #annotate 
        if df is not None:
            dfs[subdir]={"data": df,}
            # print(df)
