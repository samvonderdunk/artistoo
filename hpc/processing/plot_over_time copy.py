...
dfs = process.get(force=options.f, picklefname=keywords.nfile("evolvables.pickle"),runs=keywords.getruns(),folder=keywords.getfoldername(),  selector=select,  sortbykeywordix=keywords.getkeywordix(), sortbylineix=keywords.getlineix(),load=options.l,verbose=options.v)
...
for path in dfs:
    ...
    g = sns.scatterplot(data=df, x='time', y='value',  hue="variable", linewidth=0, palette="colorblind", ax=ax, s=0.5, alpha=0.01, rasterized=True) 
    ...
    plt.savefig(keywords.nfile("evolvables/norm/scatter"+ path[-4:] +".svg"), dpi=600)
    plt.savefig(keywords.nfile("evolvables/norm/scatter"+ path[-4:] +".png"), dpi=600)
    plt.close()
