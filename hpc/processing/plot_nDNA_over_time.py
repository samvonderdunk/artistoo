...
def select(timestep, time):
    ...
    for hostId, host in timestep.items():
        hostout = {"type":'host', "time":time, 'vol':host['vol'], 'total_vol':host['total_vol'],"id": hostId}
        hostout.update(host['evolvables'])
        hostdna = 0
        for mitoId, mito in host['subcells'].items():
        #    out.append({"type":'mito', "time":time, 'n DNA':mito['n DNA'], "id":mitoId})
            hostdna += mito['n DNA']
...
dfs = process.get(picklefname=keywords.nfile("ndna.pickle"),runs=keywords.getruns(),force=options.f, folder=keywords.getfoldername(), selector=select,  verbose=options.v,   sortbykeywordix=keywords.getkeywordix(), sortbylineix=keywords.getlineix())
...
if True:
    for path in dfs:
        ...
        g = sns.scatterplot(data=df, x='time', y='n DNA', ax=ax, s=0.2, alpha=0.3, linewidth=0)
        # g2 = sns.scatterplot(data=df, x='time', y='mito_vol', ax=ax, s=0.5, alpha=0.1)
        # entries = []

        title = "n dna through time\n"
        for key, val in dfs[path].items():
            if key != 'data':
                title += (key + " = " + str(val) + '\n')
        ...
        plt.savefig(keywords.nfile("ndna/"+ path[-4:] +".png"), dpi=1200)
        plt.close("all")
